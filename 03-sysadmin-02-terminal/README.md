...
1. Бывает, что есть необходимость переместить запущенный процесс из одной сессии в другую. Попробуйте сделать это, воспользовавшись `reptyr`. Например, так можно перенести в `screen` процесс, который вы запустили по ошибке в обычной SSH-сессии.
	...
	>### Запусти `nc` на прослушку порта в фоновом режиме

	```
	vagrant@vagrant:~$ echo "hi" | nc -l 8080 &
	[1] 3668
	vagrant@vagrant:~$
	```

	>### Запустим `screen` и с помощью `reptyr` перенесем процесс вдругую сессию, преде этим не забыв поправить параметр в `/etc/sysctl.d/10-ptrace.conf` иначе ничего не получится.
	...
	vagrant@vagrant:~$ reptyr 3668
	[detached from 3703.pts-2.vagrant]
	[detached from 3703.pts-2.vagrant]
	...
	PS D:\netology\vagrant>
	```
	>### Теперь провери работает ли наш процесс

	```
	PS D:\netology\vagrant> vagrant ssh
	vagrant@vagrant:~$ nc 127.0.0.1 8080
	hi
	```
...
1. `sudo echo string > /root/new_file` не даст выполнить перенаправление под обычным пользователем, так как перенаправлением занимается процесс shell'а, который запущен без `sudo` под вашим пользователем. Для решения данной проблемы можно использовать конструкцию `echo string | sudo tee /root/new_file`. Узнайте что делает команда `tee` и почему в отличие от `sudo echo` команда с `sudo tee` будет работать.
	...
	>### Команда `tee` используется для паралельного вывода в файл и `stdout`, и обладает правами для записи в `root` так как запускается из под `root` пользователя, в свою очередь перенаправелнием вывода заниматеся `bash` который работает из под не привелигированного пользователя.
